...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float modTrail = 0.001;

    // Trail blaze
    vec4 trail = mix(dynamicAccentColor, fragColor, 1.0 - smoothstep(0.0, sdfTrail + modTrail, 0.001));
    trail = mix(dynamicTrailColor, trail, 1.0 - smoothstep(0.0, sdfTrail + modTrail, 0.0005));
    trail = mix(trail, dynamicTrailColor, step(sdfTrail + modTrail, 0.0));

    // Cursor blaze
    trail = mix(dynamicAccentColor, trail, 1.0 - smoothstep(0.0, sdfCurrentCursor + 0.0005, 0.001));
    trail = mix(dynamicTrailColor, trail, 1.0 - smoothstep(0.0, sdfCurrentCursor + 0.0005, 0.0005));
